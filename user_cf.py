...
def user_cf(user_id, item_id, user2suser, user2score, topk):
    ...
    for similar_user, similarity in user2suser[user_id][:topk]:  # 遍历相似的topk个其他user
        score_from_similar_user = user2score[similar_user][item_id]  # 获取相似user对这个item的评分
        pred_score += score_from_similar_user * similarity  # 将评分乘上相似权重
        if score_from_similar_user != 0:
            count += 1
    pred_score /= count + 1e-5  # 分值平均化
    return pred_score
...
def movie_recommand(user_id, user2suser, user2score, item2name, topk=16):
    ...
    for item_id in possible_items:
        ...
        result.append([item2name[item_id + 1], score])
